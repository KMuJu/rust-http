...
mod tests {
    ...
    use crate::message::{Method, error::BodyError, test_utils::batch_reader::BatchReader};
    ...
    async fn test_request_connection() -> Result<(), RequestError> {
        ...
        let reader = StreamReader::new(c);
        let mut connection = Connection::<_, _, Request>::new(reader, writer);
    ...
    async fn test_request_connection_no_body() -> Result<(), RequestError> {
        ...
        let writer = Cursor::new(input.to_vec());
        let reader = StreamReader::new(batch_reader);
        let mut connection = Connection::<_, _, Request>::new(reader, writer);
